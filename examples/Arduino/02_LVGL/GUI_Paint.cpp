...
{
 const uint8_t* p_text = (uint8_t*)pString;
...
void Paint_DrawImage(const uint8_t *image, uint16_t xStart, uint16_t yStart, uint16_t W_Image, uint16_t H_Image) 
{
    int i,j; 
    for(j = 0; j < H_Image; j++){
        for(i = 0; i < W_Image; i++){
...
void Paint_DrawImage1(const uint8_t *image, uint16_t xStart, uint16_t yStart, uint16_t W_Image, uint16_t H_Image) 
{
    int i,j; 
		for(j = 0; j < H_Image; j++){
			for(i = 0; i < W_Image; i++){
    ...
    uint32_t Addr = 0;
		for (y = 0; y < Paint.HeightByte; y++) {
				for (x = 0; x < Paint.WidthByte; x++) {//8 pixel =  1 byte
						Addr = x + y * Paint.WidthByte ;
						Paint.Image[Addr] = \
						(uint8_t)image_buffer[Addr+ (Paint.HeightByte)*Paint.WidthByte*(Region - 1)];
				}
		}
}
